export const analyzeResume = async (req, res) => {
    try {
        ...
        const matchedSkills = jdWords.filter((word) => resumeWords.includes(word));
        ...
        const score = Math.min(
            ...
            Math.floor((uniqueMatched.length / jdWords.length) * 100),
        );

        // MISSING SKILLS

        const missingSkills = jdWords.filter((word) => !resumeWords.includes(word));
